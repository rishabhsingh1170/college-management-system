...
import { FaBullhorn, FaCalendarAlt, FaExclamationCircle, FaPlus, FaTools, FaBell, FaTrash, FaUserGraduate, FaChalkboardTeacher } from "react-icons/fa";
...
const NotificationAdmin = () => {
  const [notifications, setNotifications] = useState({ studentNotifications: [], facultyNotifications: [] });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoadingForm(true);
    try {
      const response = await postRequest("/admin/notifications/publish", formData);
      toast.success(response.data.message);
      setFormData({ title: '', message: '', user_type: 'student' });
      fetchNotifications();
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to publish notification.");
    } finally {
  ...
  const confirmDelete = async () => {
    try {
      ...
      await deleteRequest(`/admin/delete-notifications/${notificationToDelete}`);
  ...
  const renderNotifications = (notificationsList) => (
    notificationsList.length === 0 ? (
      <div className="text-center text-gray-500 p-4">No notifications to display.</div>
    ) : (
      <div className="space-y-4">
        {notificationsList.map((notification) => {
          const { icon, color } = getIconAndColor(notification.type);
          return (
            <div key={notification.id} className="flex items-start space-x-4 p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors duration-200">
              ...
              <button onClick={() => handleDeleteClick(notification.notification_id)} className="flex-shrink-0 text-red-500 hover:text-red-700 transition-colors">
                <FaTrash />
              </button>
            </div>
          );
      ...
      <div className="mb-8 p-6 bg-gray-50 rounded-xl shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-3">
            <label htmlFor="user_type" className="font-semibold text-gray-700">Publish to:</label>
            <select
              id="user_type"
              name="user_type"
          ...
          >
            {loadingForm ? <span>Publishing...</span> : <span>Publish Notification</span>}
          </button>
        </form>
      </div>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-4">
              <button onClick={() => setShowConfirmModal(false)} className="px-4 py-2 rounded-lg text-gray-600 hover:bg-gray-100 transition-colors">
                Cancel
              </button>
              <button onClick={confirmDelete} className="px-4 py-2 rounded-lg bg-red-600 text-white font-bold hover:bg-red-700 transition-colors">
                Delete
              </button>
            </div>
          </div>
